...
<!--      <el-button size="small" type="success" @click="submitUpload('upload1')">上传到服务器</el-button>-->
      <el-button size="small" type="warning" @click="cleanUp('upload1')">清空</el-button>
      <el-form-item>{{ value.swiperImages }}</el-form-item>
...
<!--      <el-button size="small" type="success" @click="submitUpload('upload2')">上传到服务器</el-button>-->
      <el-button size="small" type="warning" @click="cleanUp('upload2')">清空</el-button>
      <el-form-item>{{ value.descImages }}</el-form-item>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // 上传按钮触发
      submitUpload(uploadRef) {
        console.log('what the fuckt')
        this.$refs[uploadRef].submit();
      },
      // 轮播图
      handleChange1(file, fileList) {
        this.value.swiperImages = getImageIds(fileList);
      },
      ...
      handleChange2(file, fileList) {
        this.value.descImages = getImageIds(fileList);
      },
  ...
  let getImageIds = fileLists => {
    let imageIds = [];
    let pic_order = 1;
    fileLists.forEach(file => {
      // if (file["response"]) 表示这个已经上传了，一个一个上传的，但是每一个上传成功都触发一次
      if (file['response']) {
        imageIds.push({order: pic_order, sm_url: file.response.result.sm_url, name: file.name});
        pic_order++;
      }
    });
    return imageIds;
